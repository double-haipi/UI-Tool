    ...
    public void Drag()
    {
        ...
        if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
        {
            ...
            if (mPlane.Raycast(ray, out dist))
            {
                ...
                //在unity5.3.6f1 pc上测试时，发现鼠标上滑，滑动出屏幕时，delta值会突变，这里做下处理
                //限制offset 的分量值如果超过0.5，就是突变的，不要移动
                //if (Mathf.Abs(offset.x) > 0.5 || Mathf.Abs(offset.y) > 0.5)
                //{
                //    Debug.LogWarning("滑动突变");
                //    return;
                //}
                mLastPos = currentPos;
